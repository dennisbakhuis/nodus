...
When a technology reaches `TRL 10+` and is the standard choice in its category, it should move off the radar. Use `Adopted` if the organisation actually took it into normal use, and `Archive` if it became mainstream elsewhere without being adopted here. The radar is for technologies that warrant deliberate scouting attention; mainstream tech doesn't.

The test is whether an open decision remains, not how mature the technology is. `Invest` means *actively funding deployment* — a decision still being made. A technology that is simply what the organisation uses, where no reader would ever act differently for its ring, is `Adopted`, not `Invest`.
