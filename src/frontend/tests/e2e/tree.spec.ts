...
async function signInAsReader(page: Page): Promise<boolean> {
  const res = await page.request
    .post(`${API}/auth/login`, {
      data: { username: "demo_reader", password: "demo" },
    })
    .catch(() => null);
  ...
  // A freshly seeded demo account has an incomplete People profile, and the
  // completion modal is deliberately undismissable — it would sit over the
  // canvas and swallow every click. Fill it in over the API instead.
  await page.request
...
async function openTree(page: Page, path = "/tree"): Promise<boolean> {
  if (!(await signInAsReader(page))) return false;
...
test.describe("Tree E2E", () => {
  test("renders the group tree with distinct node kinds", async ({ page }) => {
    ...
    expect(await page.locator("[data-kind='labelGroup']").count()).toBeGreaterThan(
      0,
    );
    expect(
  ...
  test("dependency mode asks for an anchor and keeps mode in the URL", async ({
    ...
    test.skip(!(await openTree(page, "/tree?mode=deps")), "demo reader unavailable");
  ...
  test("an anchored lineage renders its level columns", async ({ page }) => {
    ...
    await expect(page.getByText("ANCHOR", { exact: true })).toBeVisible({ timeout: 10_000 });
